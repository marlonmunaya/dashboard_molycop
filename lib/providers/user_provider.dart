...
////////User Login
  ...
  // Future<List<DevicesModel>> fetchProducts() async {
  //   var result = await _api.getDataCollection();
  //   products = result.documents
  //       .map((doc) => DevicesModel.fromSnapshot(products))
  //       .toList();
  //   return products;
  // }
  ...
  void getcollectionmolycop (start,end) async {
    ...
    final snapshot = await Firestore.instance.collection('users')
    .where('fecha',isGreaterThanOrEqualTo: start)
    .where('fecha',isLessThanOrEqualTo:end)
    .orderBy('fecha',descending: true)
    .getDocuments();
    _model = snapshot.documents.map((e) =>DevicesModel.fromSnapshot(e)).toList();
    print(_model);
    notifyListeners();
