import 'package:molycop_dashboard/Widget/drawer_area.dart';
import 'dart:html' as html;
import 'dart:js' as js;
import 'dart:convert';
import 'package:molycop_dashboard/Widget/linechart.dart';
import 'package:molycop_dashboard/Widget/tabla.dart';
import 'package:molycop_dashboard/pages/selection.dart';
import 'package:molycop_dashboard/providers/user_provider.dart';
import 'package:molycop_dashboard/models/area.dart';
import 'package:molycop_dashboard/pages/setting.dart';
...
class _MyAppState extends State<HomeApp> {
  ...
  List<DevicesModel> datos = [];
  int data = 0;
  DateTime _startDate = DateTime.now().subtract(Duration(days: 1));
  DateTime _endDate = DateTime.now();
  String csv;
  List<charts.Series<DevicesModel, DateTime>> _createSampleData(
  ...
  Future displayDateRangePicker(BuildContext context) async {
    ...
    if (picked != null && picked.length == 2) {
      ...
      //  Provider.of<UserState>(context, listen: false).getcollectionacme(_startDate,_endDate);
       Provider.of<UserState>(context, listen: false).getcollectionmolycop(_startDate,_endDate);
      });
    }
  }
          ...
          onPressed: () async {
            setState(() {
              ...
              // as html.AnchorElement
                ...
                anchor.download = 'molycop-$_endDate.csv';
  ...
  Widget _buildMessagesPage(userstate, BuildContext context) {
    List<DevicesModel> model = userstate.model;
    if (model != null) {
      datos = model;
    }
    return SingleChildScrollView(
      child: Column(
        children: <Widget>[
          ...
          SelectionCallbackExample(_createSampleData(datos),
              animate: true
          ),
